...
for i in range(y0.shape[0]):
    ...
    ax[1].plot(x, y1[i], label=str(i))
ax[0].set_ylabel("Amplitude [dBFS]")
ax[0].set_title("port %i" % (IN_PORT_1))
ax[0].grid()
ax[1].set_xlabel("Offset frequency [MHz]")
ax[1].set_ylabel("Amplitude [dBFS]")
ax[1].set_title("port %i" % (IN_PORT_2))
ax[1].grid()
ax[1].legend()
utils.show(plt, fig)
